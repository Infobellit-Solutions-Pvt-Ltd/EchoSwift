...
USER_COUNTS=(1 3 10)
# (1 3 10 30 100)
INPUT_TOKENS=(32 64 128 256 512)
...
for u in "${USER_COUNTS[@]}"; do
    ...
    for input_token in "${INPUT_TOKENS[@]}"; do
        ...
        python3 avg_locust_results.py --input_csv_filename "$USER_FILE" --output_csv_filename "$USER_DIR/avg_${input_token}_input_tokens.csv" --tokens "${OUTPUT_TOKENS[@]}"
    done
