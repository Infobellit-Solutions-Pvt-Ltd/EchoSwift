...
def get_connection(config_file):
    ...
    RABBITMQ_HOST = config.get("mqtt_ip",10.216)
...
def send_message(channel, queue_name, msg):
...
def binary_search_user_count(config_file, low, high, result_dir):
    ...
    while low < high:
        ...
        try:
            ...
            binary_s_report_path.parent.mkdir(parents=True, exist_ok=True)
            report_data = {
                "Request": Request,
                "concurrent_user": [mid],
                "ttft": int(ttft),
                "token_latency": int(latency_per_token),
                "tokens_per_sec": round(total_throughput)
            }

            df = pd.DataFrame(report_data)
            df.to_csv(binary_s_report_path, index=False)
            print(f"Binary Search report generated at {binary_s_report_path}")
            send_data(channel, topic_name, binary_s_report_path)
...
def send_data(channel, topic_name, result_dir):
    ...
    # Convert DataFrame to JSON and parse it into a list of dictionaries
    json_data = json.loads(df.to_json(orient="records"))

    #metrics = {}  # Dictionary to store key-value pairs for each column

    for record in json_data:
        # Round numeric values and send each record individually
        formatted_data = {key: round(value, 2) if isinstance(value, (int, float)) else value
...
def run_benchmark_with_incremental_requests(config_file, optimal_user_count, result_dir):
    ...
    try:
        ...
        while True:
            ...
            ttft, latency_per_token, latency, throughput, total_throughput = extract_metrics_from_avg_response(
                config_file, result_dir, optimal_user_count)
            metrics = extract_metrics_from_avg_response(config_file, result_dir, optimal_user_count)
            if not all([ttft, latency_per_token, latency, throughput]):
            ...
            print(f"Continuous Run - User Count: {optimal_user_count}, TTFT: {ttft} ms, "
                  ...
                  f"Throughput: {throughput} tokens/second")																		
...
def adjust_user_count(config_file, result_dir):
    ...
    while True:
        ...
        try:
            ...
            report_data = {
                "Request": Request,
                "concurrent_user": [user_count],
                "ttft": int(ttft),
                "token_latency": int(latency_per_token),
                "tokens_per_sec": round(user_count * throughput, 2)
            }
